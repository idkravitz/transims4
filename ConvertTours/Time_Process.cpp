...
void ConvertTours::Time_Processing (File_Group *group)
{
	...
	Diurnal_Target_Array *diurnal_data = new Diurnal_Target_Array (0);

	//---- read the trip time into memory ----

	group->Diurnal_Data (diurnal_data);
	...
	for (i=1; i <= num_shares; i++) {
		...
		for (j=0; j < num; j++) {
			...
			for (t = 1; t <= num_rec; t++) {
				...
				diurnal_ptr = diurnal_data->Record (t1);
