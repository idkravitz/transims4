...
{
	int i, j, k, k1, num, bin,  volume, old_vol, percent, max_lanes, lanes, tod, tod_list;
	...
	for (link_ptr = link_data.First (); link_ptr; link_ptr = link_data.Next ()) {
		...
		for (i=0; i < 2; i++) {
			...
			for (j=0; j < num; j++) {
				...
				old_vol = perf_ptr->Volume2 (j+1);

				if (old_vol < min_volume) continue;
				...
				bin = abs (volume - old_vol) * 10 * RESOLUTION / old_vol;
...
void LinkSum::Volume_Change_Header (void)
{
	...
	Print (2, "%30cVolume Change * 100 / Previous Volume", BLANK);
