...
void ActivityPattern::Household_Match (HHold_Data *household_ptr)
{
	int i, hh, hhold, relate, age, group, group1, group2, work, gender, match, ndata;
	...
	for (pop_ptr = survey_pop_data.Get_GE (hhold); pop_ptr; pop_ptr = survey_pop_data.Next_Key ()) {
		...
		data_ptr->relate = pop_ptr->Relate ();
		data_ptr->age = age;
